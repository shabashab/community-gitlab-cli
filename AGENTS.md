...
- `internal/cli/mr_discussions.go`: `mr discussions` (thread list) and `mr discussion` (single-thread view) commands — fetch-all pagination over the discussions API, thread summarization (`summarizeDiscussion`), the client-side filter/sort/page pipeline, and discussion short-ID prefix resolution (`resolveDiscussionRef`).
...
- `internal/cli/mr_diff.go`: `mr diff` command suite — changed-file summaries, raw patch output, and filesystem review-bundle export (`manifest.toon`, `files.toon`, `patch.diff`, `diffs/`, `old/`, `new/`).
- `internal/cli/mr_diffpos.go`: smart diff positioning — a pure unified-diff hunk parser mirroring GitLab's cursor semantics, `parseLineSpec` (`N`/`A:B`), `diffLineCode` fabrication, and `resolveCommentPosition` building full GitLab position objects from merge request diff refs and file diffs.
...
- `internal/cli/mr_table.go`: standard-mode merge request, diff, draft-note, and discussion table rendering; the only file importing go-pretty.
